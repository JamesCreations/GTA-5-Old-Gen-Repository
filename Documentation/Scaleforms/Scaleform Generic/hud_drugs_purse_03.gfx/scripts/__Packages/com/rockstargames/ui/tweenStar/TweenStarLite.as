class com.rockstargames.ui.tweenStar.TweenStarLite
{
   ...
   function TweenStarLite(_mytarget, _duration, _vars, _isTween)
   {
      ...
      this.vars = _vars;
      this.duration = _duration * com.rockstargames.ui.tweenStar.TweenStarLite.frameRate;
      this.startTime = this.vars.delay * (- com.rockstargames.ui.tweenStar.TweenStarLite.frameRate) || 0;
      this.ease = com.rockstargames.ui.tweenStar.TweenStarLite.linearEase;
      this.props = [];
      if(_isTween)
      {
   ...
   function updateAll()
   {
      ...
      var _loc2_ = this.startTime / this.duration;
      _loc2_ = Math.min(_loc2_,1);
      for(var _loc3_ in this.props)
      {
         if(_loc2_ > 0)
         {
            this.target[this.props[_loc3_][0]] = this.ease(this.startTime,this.props[_loc3_][1],this.props[_loc3_][2] - this.props[_loc3_][1],this.duration);
         }
      }
      if(_loc2_ == 1)
      {
