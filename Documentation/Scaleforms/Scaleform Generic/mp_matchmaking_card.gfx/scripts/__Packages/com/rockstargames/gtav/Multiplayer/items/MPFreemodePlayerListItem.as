class com.rockstargames.gtav.Multiplayer.items.MPFreemodePlayerListItem extends com.rockstargames.gtav.pauseMenu.pauseMenuItems.PauseMenuBaseItem
{
   ...
   function setRightText(rightText, isJob)
   {
      ...
      if(isJob)
      {
         this.jobPointsTF.text = this.__get__data()[6];
      }
      else
      {
         com.rockstargames.ui.utils.UIText.setSizedText(this.scoreTF,this.__get__data()[6],true);
      }
   }
   ...
   function ADD_TXD_REF(txd, txn)
   {
      ...
      if(this.avatarImg.textureDict == txd)
      {
         ...
         {
            this.avatarImg.init("mp_matchmaking_card",txd,txn,25,25);
      ...
      else
      {
         ...
         this.avatarImg.init("mp_matchmaking_card",txd,txn,25,25);
         var _loc3_ = this.avatarImg.splitPath(String(this.avatarImg),3);
